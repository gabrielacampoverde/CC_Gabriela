   ...
   function Cell($w, $h = 0, $txt = "", $border = 0, $ln = 0, $align = "", $fill = false, $link = "") {
      ...
      if ($txt !== "") {
         ...
         if ($link) {
            ...
            $this->Link($this->x + $dx, $this->y + 0.5 * $h - 0.5 * $this->FontSize, $wlink, $this->FontSize, $link);
         }
      }
      if ($s) {
         $this->_out($s);
...
class Table extends FPDF {
   ...
   function Row1($ii, $lcNumRow, $data, $last) {
      # heght
      ...
      for ($i = 0; $i < count($data); $i++) {
         ...
         if ($i == 0 || $i == 3) {
            $this->Rect($x, $y, $w, $h);
            $this->MultiCell($w, 0.5, $data[$i], 0, "J");
         } elseif ($i == 1 || $i == 2) {
            $this->Rect($x, $y, $w, $h);
            $this->MultiCell($w, 0.5, $data[$i], 0, "C");
         } elseif ($i == 4) {
            /*$this->Rect($x,$y,$w,$h);
             $this->MultiCell($w,0.5,$data[$i],0,'C');*/
            //$lcHeight = $lcNumRow *($h * 0.5);
            $lcposition = $lcNumRow / 2;
            if ($lcNumRow == $ii) {
               $this->Rect($GLOBALS['$X1'], $GLOBALS['$Y1'], $w, $lcNumRow);
               if ($lcNumRow == 1) {
                  $this->MultiCell($w, 0.5, $data[$i], 0, "C");
               } else {
                  $this->MultiCell($w, 0.5, "", 0, "C");
               }
            } elseif ($ii == (int) ceil($lcposition)) {
               //$this->Rect($x,$y,$w,$h);
               $this->MultiCell($w, 0.5, $data[$i], 0, "C");
            } else {
               $this->MultiCell($w, 0.5, "", 0, "C");
            ...
            if($lcNumRow==$ii){
               if($lcNumRow == 1){
                  $this->MultiCell($w,0.5,$data[$i],'LRB','C');
               } else{
                  $this->MultiCell($w,0.5,'','LRB','C');
               }
            } elseif($ii==1){
               //$this->Rect($x,$y,$w,$h);
               $this->MultiCell($w,0.5,$data[$i],'LTR','C');
            } else{
               $this->MultiCell($w,0.5,'','LR','C');
            }
         }*/
...
class PDF extends Table {
   ...
   function DoStyle($ind) {
      ...
      $this->SetFont($this->TagStyle[$ind]["family"], $this->TagStyle[$ind]["style"], $this->TagStyle[$ind]["size"]);
