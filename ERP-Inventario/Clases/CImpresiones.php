...
//      $loSqlIns->omDisconnect();
   ...
   protected function mxValParamUsuario($p_oSql, $p_cModulo = '00N') {
      $lcSql = "SELECT cEstado FROM V_S01PCCO WHERE cCencos = 'UNI' AND cCodUsu = '{$this->paData['CCODUSU']}' AND cModulo = '00N'";
      ...
      $lcSql = "SELECT cEstado FROM V_S01PCCO WHERE cCencos = '{$this->paData['CCENCOS']}' AND cCodUsu = '{$this->paData['CCODUSU']}' AND cModulo = '$p_cModulo'";
      $RS = $p_oSql->omExec($lcSql);
      $laFila = $p_oSql->fetch($RS);
      if (!isset($laFila[0]) or empty($laFila[0])) {
   ...
   public function omListarImpresionesOtros() {
      $llOk = $this->mxValListarImpresionesOtros();
      if (!$llOk) {
         return false;
      }
      $loSql = new CSql();
      ...
      $llOk = $this->mxListarImpresionesOtros($loSql);
      $loSql->omDisconnect();
      return $llOk;
   }

   protected function mxValListarImpresionesOtros() {
      if (!isset($this->paData['CCODUSU']) || !preg_match('/^[0-9A-Z]{4}$/', $this->paData['CCODUSU'])) {
         $this->pcError = 'VALOR DEL CÓDIGO DE USUARIO ES INVÁLIDO';
         return false;
      }
   ...
   protected function mxListarImpresionesOtros($poSql) {
      $lcSql = "SELECT cidimpr, TO_CHAR(tsolici, 'YYYY-MM-DD'), cestado, TO_CHAR(taproba, 'YYYY-MM-DD'), TO_CHAR(trecojo, 'YYYY-MM-DD') FROM B11MIMP 
         WHERE CTIPSOL = 'F' AND CESTADO = 'C' ORDER BY taproba DESC";
      $RS = $poSql->omExec($lcSql);
      $this->paDatos = [];
      while($laFila = $poSql->fetch($RS)) {
         $laDatos[] = ['CIDIMPR'=> $laFila[0], 'TSOLICI'=> $laFila[1], 'CESTADO'=> $laFila[2], 'TAPROBA'=> $laFila[3], 'TRECOJO'=> $laFila[4]];
      }
   ...
   public function mxListarImpresionesTrabajador() {
      ...
      $loSql->omDisconnect(); //J.C.F. Agregó
      $loSqlIns = new CSql();
      $llOk = $loSqlIns->omConnect(2);
      if (!$llOk) {
         $this->pcError = $loSqlIns->pcError;
   ...
   protected function mxAgregarDetalleImpresionDocente($poSql) {
      ...
      if ($this->paData['CIDIMPR'] != '') {
         ...
         if (!$RS) {
//            $this->pcError = $lcSql;
            $this->pcError = "Ha ocurrido un error al agregar la solicitud. Contacte con soporte del ERP.[2]".$lcSql;
            return false;
         }
      }
...
//      $lmDatos = json_decode($laFila[0], true);
   ...
   protected function mxConfirmarAprobacion($p_oSql) {
      ...
      $laEmails = $this->paData['CEMAIL'];
      //$laEmails = '74606599@ucsm.edu.pe';
      //$laCopia = '';
      $loEmail->paData = ['AEMAIL'=> $laEmails, 'CBODY'=> 'Se ha Imprimido su solicitud con codigo: '.$this->paData['CIDIMPR']];
      $loEmail->omSend();
      return true;
   }
   ...
   protected function mxRechazarAprobacion($p_oSql) {
      ...
      $loEmail = new CEmailImpresionesRechazar();
      //$laEmails = $this->paData['CEMAIL'];
      $laEmails = '74606599@ucsm.edu.pe';
      $Rechazo = 'Su peticion de impresiones con numero de solicitud: '.$this->paData['CIDIMPR'].',fue rechazado por el siguiente motivo: '.$this->paData['CRECHAZ'];
      $loEmail->paData = ['AEMAIL'=> $laEmails, 'CBODY'=> $Rechazo];
      $loEmail->omSend();
      return true;
   }
   ...
   public function omListaImpresorasOtros() {
      $loSql = new CSql();
      $llOk = $loSql->omConnect(0);
      if (!$llOk) {
         $this->pcError = $loSql->pcError;
         return false;
      }
      $llOk = $this->mxListaImpresorasOtros($loSql);
      $loSql->omDisconnect();
      if (!$llOk) {
         $this->pcError = $loSql->pcError;
         return false;
   ...
   protected function mxListaImpresorasOtros($p_oSql) {
      $lcSql = "SELECT mdatos FROM S01TVAR WHERE cnomvar = 'AIMPRCOD'";
      $RS = $p_oSql->omExec($lcSql);
      $laFila = $p_oSql->fetch($RS);
      if ($laFila[0] == null) {
      ...
      $this->paDatos['ALSTIMP'] = $laDatos;
      return true;
   }
   
   // ------------------------------------------------------------
   // Agregar Detalle Pedidos de Impresiones
   // 2023-09-04           J.C.F. CREACION
   // ------------------------------------------------------------
   public function omAgregarDetalleImpresionOtros() {
      $llOk = $this->mxValParamAgregarDetalleImpresionOtros();
      if (!$llOk) { 
         return false;
      }
      $loSql = new CSql();
      ...
      //VALIDACION DE TIPO DE USUARIO -- OTROS
      $llOk = $this->mxAgregarDetalleImpresionOtros($loSql);
      $loSql->omDisconnect(); 
      if (!$llOk) {
         $this->pcError = $loSql->pcError;
         $loSql->rollback();
   ...
   protected function mxValParamAgregarDetalleImpresionOtros() {
      if (!isset($this->paData['CCODUSU']) || !preg_match('/^[0-9A-Z]{4}$/', $this->paData['CCODUSU'])) {
         $this->pcError = 'VALOR DEL CÓDIGO DE USUARIO ES INVÁLIDO';
         return false;
      } elseif (!isset($this->paData['CCENCOS']) || !preg_match('/^[0-9A-Z]{3}$/', $this->paData['CCENCOS'])) {
   ...
   protected function mxAgregarDetalleImpresionOtros($poSql) {
      //AGREGA SOLICITUD EN LA TABLA MAESTRO IMPRESION
      $lcCorrel = '00000';
      $lcSql = "SELECT CASE WHEN MAX(CIDIMPR) IS NULL THEN '00000' ELSE MAX(CIDIMPR) END AS CCORREL FROM B11MIMP";
      $RS = $poSql->omExec($lcSql);
      if (!$RS) {
         $this->pcError = "ERROR AL GENERAR CODIGO DE SOLICITUD";
         return false;
      }
      $laFila = $poSql->fetch($RS);
      if ($laFila[0] != null) {
      ...
      $this->paData['CIDIMPR'] = fxCorrelativo($lcCorrel);
      //INSERTAR MAESTRO IMPRESIONES {$this->paData['CCENCOS']}
      $lcSql = "INSERT INTO B11MIMP (CIDIMPR, CTIPSOL, CCODUSU, TSOLICI, TRECOJO, CESTADO, CCENCOS, CUSUAPR, TAPROBA, TIMPRES, CUSUCOD, TMODIFI)
                  VALUES ('{$this->paData['CIDIMPR']}', 'F', '{$this->paData['CCODUSU']}', NOW(), NOW(), 'C', '000', 
                  '{$this->paData['CCODUSU']}', NOW(), NOW(), '{$this->paData['CCODUSU']}', NOW())";
      $RS = $poSql->omExec($lcSql);
      if (!$RS) {
         $this->pcError = "HA OCURRIDO UN ERROR AL MOMENTO DE INGRESAR LA SOLICITUD.";
         return false;
      }
      foreach ($this->paData["ADETALL"] as $laData) {
         if ($laData['BCOLOR'] == 'on') {
            $laData['BCOLOR'] = 'S';
         } else {
            $laData['BCOLOR'] = 'N';
         }
         if ($laData['BANILLA'] == 'on') {
            $laData['BANILLA'] = 'S';
         } else {
            $laData['BANILLA'] = 'N';
         }
         $lmDatos = ["NCARAS"=> $laData['NCANPAG'], 'CDOBCAR'=> $laData['CTIPIMP'], 'CCOLOR'=> $laData['BCOLOR'], 
                     'CTIPEVA'=> 'O', 'CDESCRI'=> strtoupper($laData['CASUNTO'])];
         $lmDatos = json_encode($lmDatos, true);
         $lcSql = "INSERT INTO B11DIMP (CIDIMPR, CCODCUR, CESTADO, CCLASE, CFORMAT, NCANTID, MDATA, CUSUCOD, TMODIFI, CCODIMP)
                     VALUES ('{$this->paData['CIDIMPR']}', '0000000', 'A', 'D', '{$laData['CTIPHOJ']}', {$laData['NCANJUG']},
                     '{$lmDatos}','{$this->paData['CCODUSU']}' , NOW(), '{$laData['CCODIMP']}')";
         $RS = $poSql->omExec($lcSql);
         if (!$RS) {
            $this->pcError = "HA OCURRIDO UN ERROR AL MOMENTO DE CREAR LA SOLICITUD";
            return false;
         }
      }
      return true; 
