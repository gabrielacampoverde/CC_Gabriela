...
require_once "Libs/fpdf/fpdf.php";   // OKOKEI PARA QUE?
...
class CParqueo extends CBase {
   ...
   protected function mxValVicerrectorado() {
      ...
      //1015
      if (!isset($this->paData['CUSUCOD']) or ($this->paData['CUSUCOD'] != '3378')) {
   ...
   protected function mxValSeguridad() {
      ...
      //1015
      if (!isset($this->paData['CUSUCOD']) or !in_array($this->paData['CUSUCOD'], ['3378','1936'])) {
   ...
   protected function mxValRrhhContabilidad() {
      ...
      //1015
      if (!isset($this->paData['CUSUCOD']) or !in_array($this->paData['CUSUCOD'], ['3378','1815','1871'])) {
         $this->pcError = 'EL USUARIO NO TIENE PERMISOS PARA REALIZAR ESTA ACCIÓN';
         return false;
      }
      return true;
   ...
   protected function mxValParamUsuario($p_oSql, $p_cModulo = '010') {
   ...
   protected function mxCargarValidarDatosParqueo() {
      ...
      while (true) {
         ...
         $llOk = $loValid && $loValid->format('Y-m-d H:i:s') == $ltFecHor;
         if (!$llOk) {
            $llOk = false;
         ...
         if (substr($lcIngSal, 0, 3) == 'ING') {
            ...
            foreach ($laDatos as $laTmp) {
               $j++;
               if ($laTmp['CPLACA'] == $lcPlaca) {
                  break;
               }
            }
            if ($j == -1) {
               $llOk = false;
               $lcLinea = (string)$i.') ERR09 - '.$lcPlaca.PHP_EOL;
               fwrite($loFile, $lcLinea);
            } else {
               $laDatos[$j]['TSALIDA'] = $ltFecHor;
            }
         } else {
            $llOk = false;
      ...
      // fclose($loFile);
      // $loXls->closeXls();   // OJOKEI EL XLSX DEBE CERRARSE SINO SE QUEDAN CON EL PUNTERO ABIERTO Y LUEGO NO SE PUEDEN DEPURAR
      if (count($laDatos) == 0) {
   ...
   protected function mxCargarValidarDatosParqueoFinal() {
      ...
      // $loXls->closeXls();   // OKOKEI, CERRAR EL XLSX
      if (count($laDatos) == 0) {
         $this->pcError = 'NO HAY DATOS PARA CARGAR';
         return false;
      }
   ...
   protected function mxValidarDatosParqueoFinal($p_oSql) {
      ...
      if (!$llOk) {
         ...
         $lcRead = fopen($lcFilErr, "r");
         $lcReadFile = fread($lcRead, filesize($lcFilErr));
         $this->pcDetErr = $lcReadFile;
         $this->pcError  = 'HAY ERRORES EN DATOS - REVISE EL LOG QUE APARECE A CONTINUACIÓN';
         return false;
      }
   ...
   # KRA Creacion
   # ----------------------------------------------------------------
   public function omGrabarDatosParqueo() {
      //$this->paData['CFILEUP'] = './Xls/MPARQUEO_CONDATOS.xlsx';
      // $llOk = $this->mxValParamGrabarDatosParqueo();
      // if (!$llOk) {
      //    return false;
      // }
      ...
      // $llOk = $this->mxValidarDatosParqueo($loSql);
      // if (!$llOk) {
      //    $loSql->omDisconnect();
      //    return false;
      // }
      $llOk = $this->mxGrabarDatosParqueo($loSql);
      if (!$llOk) {
         $loSql->omRollback();
      }
      $loSql->omDisconnect();
   ...
   // protected function mxValParamGrabarDatosParqueo() {
   //    return true;
   // }

   protected function mxGrabarDatosParqueo($p_oSql) { 
      ...
      $lcSql = "DELETE FROM B11DPAR WHERE TO_CHAR(TENTRAD, 'YYYY-MM-DD') BETWEEN '$ldFecIni' AND '$ldFecFin'";
      // $lcSql = "DELETE FROM B11DPAR WHERE TO_CHAR(TENTRAD, 'YYYY-MM-DD') = '{$this->paData['DFECHA']}'";
      // $lcSql = "DELETE FROM B11DPAR WHERE TO_CHAR(TENTRAD, 'YYYY-MM-DD') = '2023-07-11'";
      $llOk = $p_oSql->omExec($lcSql);
      if (!$llOk) {
         $this->pcError = "NO SE PUDO ELIMINAR ...";
         return false;
      }
      ...
      foreach ($this->paDatos as $laFila) {
         // $lcSql = "SELECT cIdenti FROM B11MPAR WHERE cNroDni = '{$laFila['CNRODNI']}' AND cplaca = '{$laFila['CPLACA']}'";
         $lcSql = "SELECT cIdenti FROM B11MPAR WHERE cplaca = '{$laFila['CPLACA']}'";
         $R1 = $p_oSql->omExec($lcSql);
         $laTmp = $p_oSql->fetch($R1);
         if (!isset($laTmp[0]) or empty($laTmp[0])) {
            ...
            $llOk = $p_oSql->omExec($lcSql);
            if (!$llOk) {
               $this->pcError = "NO SE PUDO INSERTAR DATO ...";
               return false;
            }
         } else {
            $lcIdenti = $laTmp[0];
   ...
   protected function mxMostrarReporteCobroParqueo2($p_oSql) {
      ...
      $lcSql = "SELECT cPeriod, cNroDni, cNombre, SUM(nTiempo), SUM(nMonto) FROM
               ...
               WHERE cPeriod = '{$lcPeriod}' AND B.CESTADO='A' ORDER BY C.cNombre) Z
               WHERE NMONTO>0 
   ...
   protected function mxCargarMostrarRegistroParqueoVicerrectorado($p_oSql) {
      // Valida que se pueda enviar a RRHH   // OJOKEI, COMO SE PUEDE HACER ESTO???  COORDINA CON KRIS Y MAÑANA LO DISCUTIMOS
      // VALIDA USUARIO RRHH
      $lcCodUsu = $this->paData['CUSUCOD'];
      $lcSql = "SELECT cCenCos FROM S01PCCO WHERE cCenCos = '02T' AND cCodUsu = '{$lcCodUsu}'";
      ...
      $lcSql = "SELECT B.cNroDni, C.cNombre, SUM(A.nMonto) FROM  B11DPER A
                INNER JOIN B11MPAR B ON B.cIdenti = A.cIdenti
                INNER JOIN S01MPER C ON C.cNroDni = B.cNroDni
                WHERE A.cPeriod = '{$lcPeriod}' AND B.cEstado = 'A' AND A.cEstado = 'B' and A.nMonto > 0 
   ...
   protected function mxValParamXlsxRegParqueo() {
      ...
      //    return false;
      // }
      if (!isset($this->paData['CBUSQUE']) || strlen(trim($this->paData['CBUSQUE'])) < 0 || strlen(trim($this->paData['CBUSALU'])) > 50) {
   ...
   // -------------------------------------------------------------------------------
   public function omMostrarRegistroRrhh() {
      ...
      $llOk = $this->mxCargarMostrarRegistroRrhh($loSql);
      if (!$llOk) {
         return false;
         $loSql->omDisconnect();
      }
      $loSql->omDisconnect();
      $llOk = $this->mxFileXlsxMostrarRegistroRrhh();
      return $llOk;
   }
   ...
   protected function mxCargarMostrarRegistroRrhh($p_oSql) {
      // Valida que se pueda enviar a RRHH   // OJOKEI, COMO SE PUEDE HACER ESTO???  COORDINA CON KRIS Y MAÑANA LO DISCUTIMOS
      // VALIDA USUARIO RRHH
      $lcCodUsu = $this->paData['CUSUCOD'];
      $lcSql = "SELECT cCenCos FROM S01PCCO WHERE cCenCos = '02T' AND cCodUsu = '{$lcCodUsu}'";
      ...
      $lcSql = "SELECT B.cNroDni, C.cNombre, SUM(A.nMonto) FROM  B11DPER A
                INNER JOIN B11MPAR B ON B.cIdenti = A.cIdenti
                INNER JOIN S01MPER C ON C.cNroDni = B.cNroDni
                WHERE A.cPeriod = '{$lcPeriod}' AND B.cEstado = 'A' AND A.cEstado = 'C' and A.nMonto > 0 
                GROUP BY B.cNroDni,  C.cNombre ORDER BY cNombre";
      $R1 = $p_oSql->omExec($lcSql);
      while ($laTmp = $p_oSql->fetch($R1)) {
         $lcSql = "SELECT cUsuInf, cEstado FROM S01TUSU WHERE cNroDni = '{$laTmp[0]}' ORDER BY cUsuInf";
   ...
   protected function mxFileXlsxMostrarRegistroRrhh() {
      $loXls = new CXls();
      $loXls->openXlsIO('ParqueoCobroRRHH', 'R');
      $loXls->sendXls(0, 'E', 1, 'Periodo: '.$this->paData['CPERIOD']);
      $i = 1;
   ...
   public function omMostrarXlsRrhh() {
      $llOk = $this->mxValXlsRrhh();
      if (!$llOk) {
         return false;
      }
      $llOk = $this->mxFileMostrarXlsRrhh();
      return $llOk;
   }

   protected function mxValXlsRrhh() {
      if (!isset($this->paData['CPERIOD']) || !preg_match('/^20[0-9]{2}-[0-9]{2}$/', $this->paData['CPERIOD'])) {
         $this->pcError = "PERIODO NO DEFINIDO O INVÁLIDO";
         return false;
      } elseif (!isset($this->paDatos)){
   ...
   protected function mxFileMostrarXlsRrhh() {
      $loXls = new CXls();
      $loXls->openXlsIO('ParqueoCobroRRHH', 'R');
      $loXls->sendXls(0, 'E', 1, 'Periodo: '.$this->paData['CPERIOD']);
      $i = 1;
