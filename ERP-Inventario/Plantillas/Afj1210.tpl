...
<head>
   ...
   <script>
      ...
      function f_Reporte(p_cIdTrnf) {
         ...
         // alert(lcSend);
         $.post("Afj1210.php", lcSend).done(function(p_cResult) {
      ...
      function f_CargarCentroResp() {
         ...
         $.post("Afj1210.php",lcSend).done(function(p_cResult) {
            ...
            for (var i = 0; i < laDatos['LADATOS'].length; i++) {
               ...
               if(laDatos['LADATOS'][i].CCENRES == '01368'){
      ...
      function f_buscarEmpleadoDes() {
         ...
         //alert(lcSend);
         $.post("Afj1210.php", lcSend).done(function(lcResult) {
            //alert(lcResult);
            var laJson = JSON.parse(lcResult);
            if (laJson.ERROR) {
               alert(laJson.ERROR);
...
<div id="header"></div>
<form action="Afj1210.php" method="post" enctype="multipart/form-data" id="poForm">
   ...
   <div class="container-fluid">
      <div class="card-header" style="background:#fbc804; color:black;">
         <div class="input-group input-group-sm d-flex justify-content-between">
            <div class="col text-left"><strong>TRANSFERENCIA REDES Y TELECOMUNICACIONES</strong></div>
         ...
         <div class="input-group mb-1">
            ...
            <select class="selectpicker form-control col-sm-12 black-input col-lg-5" data-live-search="true" id="pcCenCos" onchange="f_CargarCentroResp();" name="paData[CCENCOS]" data-live-search="true">
               {foreach from=$saCenCos item=i}                        
                  <option value="{$i['CCENCOS']}" {if $i['CDESCOS'] eq 'REDES Y TELECOMUNICACIONES'} selected{/if}> {$i['CCENCOS']} - {$i['CDESCOS']}</option>
               {/foreach}
            </select>
            <div class="input-group-prepend col-lg-2 px-0">
